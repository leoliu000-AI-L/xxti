...
function getAnsweredCount(visibleQuestions) {
  return visibleQuestions.filter((question) => isQuestionAnswered(question)).length;
}

function syncCurrentQuestionIndex(visibleQuestions) {
...
function resetQuestionUi() {
  ...
  if (ui.submitBtn) {
    ...
    ui.submitBtn.textContent = '下一题';
  }
  if (ui.testHint) ui.testHint.textContent = '';
}
...
function renderQuestionNav(visibleQuestions) {
  ...
  visibleQuestions.forEach((question, index) => {
    const navButton = makeEl('button', 'question-nav-item');
    navButton.type = 'button';
    navButton.dataset.questionId = question.id;
    navButton.dataset.index = String(index);
    navButton.setAttribute('aria-label', `Question ${index + 1}`);
...
function updateActionButtons(visibleQuestions) {
  const total = visibleQuestions.length;
  ...
  const answeredCurrent = isQuestionAnswered(currentQuestion);
  const answeredAll = getAnsweredCount(visibleQuestions) === total && total > 0;

  if (ui.backBtn) {
    ui.backBtn.disabled = isFirst;
    ui.backBtn.textContent = '上一题';
  }

  if (ui.submitBtn) {
    ui.submitBtn.textContent = isLast ? '查看结果' : '下一题';
    ui.submitBtn.disabled = !answeredCurrent || (isLast && !answeredAll);
  }
...
function buildOption(question, option, optionIndex) {
  ...
  const wrapper = makeEl('div', 'option-body');
  const letter = makeEl('strong', 'option-letter', ['A', 'B', 'C', 'D'][optionIndex] || String(optionIndex + 1));
  const text = makeEl('div', 'option-text', option.label);
...
function renderCurrentQuestion(visibleQuestions) {
  clearNode(ui.questionList);
  const question = visibleQuestions[state.currentQuestionIndex];
  if (!question) return;

  const meta = question.special
    ? (theme.specialQuestionLabel || 'Hidden Branch')
    : (theme.hideDimensionLabels
        ? (theme.regularQuestionLabel || data.dimensionMeta[question.dim].name)
        : data.dimensionMeta[question.dim].name);

  const metaRow = makeEl('div', 'question-meta');
  const order = makeEl(
    'span',
    'question-order',
    `${theme.questionPrefix || '第'} ${state.currentQuestionIndex + 1}${theme.questionSuffix || '题'}`
  );
  const metaText = makeEl('span', 'question-dim', meta);
  ...
  ui.questionList.append(metaRow, questionText, optionList);
}

function renderQuestions() {
  const visibleQuestions = getVisibleQuestions();
  syncCurrentQuestionIndex(visibleQuestions);
  renderProgress(visibleQuestions);
  renderQuestionNav(visibleQuestions);
  renderCurrentQuestion(visibleQuestions);
  updateActionButtons(visibleQuestions);
}
...
function goNextStep() {
  ...
  const isLast = state.currentQuestionIndex === visibleQuestions.length - 1;
  const answeredAll = getAnsweredCount(visibleQuestions) === visibleQuestions.length;

  if (isLast) {
    if (answeredAll) renderResult();
    return;
  }
...
ui.backBtn?.addEventListener('click', goPrevQuestion);
ui.submitBtn?.addEventListener('click', goNextStep);
